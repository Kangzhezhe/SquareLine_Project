...
//MQTT三元组
#define ClientID "6687def786799a26c45e94d5_window_0_0_2024080809"
#define Username "6687def786799a26c45e94d5_window"
#define Password "7368cf6cdf8b47e9466a06495ed9abfe6f3867e7bbf49f023b23c78bc5c96105"//密文 
...
char mqtt_message[202400];//数据缓存区
char mqtt_message1[202400];//数据缓存区
extern int max_num;
...
int main_mqtt()
{
    ...
    // HANDLE hThread1 = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)MQTT_Ping, NULL, 0, NULL);
	// if (hThread1 == NULL) {
	// 	printf("CreateThread1 failed.\n");
	// 	return 1;
	// }
...
// 处理服务器下发的数据
void ReceiveDataThread::ReceiveData(void)
{
	// 接收数据
	char buffer[1024];
	char request_id[100];
			...
			if (strstr((char*)&buffer[5], "test/M2M"))
			{
				...
				if (p)
				{
					if (q)
                        ...
                        x = 2;
				}else if(strstr((char*)&buffer[5], "record"))
                {
                    x = 3;
                }
                printf("mingling x=%d\r\n",x);
				if (x == 1)
				{
					...
					addTableData(faceid, tiwen, xueyang, xinlv , (char*)"care");
				}else if(x == 2){
                    parseAndGetValue(buffer, "faceid", faceid);
					printf("faceid:%s\r\n", faceid);
					parseAndGetValue(buffer, "yao", yao);
					printf("yao:%s\r\n", yao);
					addTableData_yao(faceid, yao , (char*)"medicine");
                }else if(x == 3){
                    parseAndGetValue(buffer, "pkg_id", pkg_id);
                    cur_num = atoi(pkg_id);
                    if(cur_num==0){
                        main1();
                        recvSize = recv(connectSocket, buffer, 1024, 0);
                        ...
                        flattenedFeatures = std::vector<char>(fileSize);

                        // Read the data from the file into the vector
                        inFile.read(flattenedFeatures.data(), fileSize);
                        inFile.close();
                        cout<<flattenedFeatures.size()<<endl;

                        sprintf(mqtt_message1, "max_num:%d,", max_num);//温度
                        strncat(mqtt_message, mqtt_message1, strlen(mqtt_message1));
                        mqtt_message1[0] = '\0';
                        sprintf(mqtt_message1, "data:");//温度
                        strncat(mqtt_message, mqtt_message1, strlen(mqtt_message1));
                        mqtt_message1[0] = '\0';
                        string t;
                        t += "num_id:" + std::to_string(cur_num) + ",";
                        t+=string(mqtt_message);
                        for (int j = 0; j < 4*128; j++)
                        {
                            t+=std::to_string(flattenedFeatures[cur_num * 4*128 + j])+",";
                        }
                        t+=";";
                        // cout<<t<<endl;
                        MQTT_PublishData((char*)POST_TOPIC, (char*)t.c_str(), 0);
                        Sleep(1000);
                    }else if(cur_num<max_num){
                        // qt进度条.setValue((cur_num/max_num*100) '%');
                        sprintf(mqtt_message1, "max_num:%d,", max_num);//温度
                        strncat(mqtt_message, mqtt_message1, strlen(mqtt_message1));
                        mqtt_message1[0] = '\0';
                        sprintf(mqtt_message1, "data:");//温度
                        ...
                        MQTT_PublishData((char*)POST_TOPIC, (char*)t.c_str(), 0);
                        Sleep(500);
                        emit progressUpdated((cur_num * 100) / max_num);
                        Sleep(500);
                        if(cur_num==max_num-1){
                            Sleep(500);
                            emit progressUpdated((max_num * 100) / max_num);
                            Sleep(500);
                        }
                        cur_num=0;
                    }else if(cur_num==max_num){
				...
				else
				{
					...
					//faceid[1] = '\0';
					queTableData(faceid,(char*)"information");
				}
				//上报数据
...
bool queTableData(char* faceid, char* tablename) {
	...
	if (!connectDB(mysql)) {
		...
		// C语言组合字符串
	snprintf(sql, SQL_MAX, "SELECT 姓名,性别,年龄,诊断,意见,用药 FROM %s WHERE faceid= %s;", tablename, faceid);
	printf("查询sql语句：%s\n", sql);
	// 查询数据
	int ret = mysql_real_query(&mysql, sql, (unsigned long)strlen(sql));
	...
	// C语言组合字符串
	snprintf(sql, SQL_MAX, "SELECT 体温,血氧,心率,时间 FROM %s WHERE faceid= %s;", (char*)"care", faceid);
	printf("查询sql语句：%s\n", sql);
	// 查询数据
	...
	// C语言组合字符串
	snprintf(sql, SQL_MAX, "SELECT 用药,时间 FROM %s WHERE faceid= %s;", (char*)"medicine", faceid);
	printf("查询sql语句：%s\n", sql);
	// 查询数据
